...
async def upload_file(file: UploadFile = File(...)):
    ...
    with open("src/uploads/nationality/" + str(id_image)+".jpg", "wb") as myfile:
...
def get_file(name_file: str):
    ...
    return FileResponse("src/uploads/nationality/" + name_file)
...
def download_file(name_file: str):
    ...
    return FileResponse("src/uploads/nationality/" + name_file, media_type="application/octet-stream", filename=name_file)

@document_nationality.delete("/delete/{name_file}")
def delete_file(name_file: str):
    try:
        #remove(getcwd() + "/src/uploads/nationality/" + name_file)
        remove("src/uploads/nationality/" + name_file)
        return JSONResponse(content={
            "removed": True
        }, status_code=200)
